...
resource "azurerm_network_interface" "network_interface_card" {
    ...
    location                  = "${var.location}"
    resource_group_name       = "${var.resource_group}"
    # network_security_group_id = "${azurerm_network_security_group.network_security_group_id}"
...
resource "azurerm_virtual_machine" "virtual_machine" {
    ...
    # primary_network_interface_id = "host0"
    primary_network_interface_id = "${element(azurerm_network_interface.network_interface_card.*.id, count.index)}"
    ...
    os_profile_linux_config {
        ...
        ssh_keys {
            path = "${element(var.ssh_path, count.index)}"
            key_data = "${file("${element(var.ssh_key, count.index)}")}"  
        
            # "${var.ssh_key}" 
            # "${file("~/ssh/idrsa.pub")}"
        }
    }
    #depends_on = ["azurerm_network_interface.network_interface_card", "azurerm_public_ip.public_ip"]
}